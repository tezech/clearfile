...
// Accept 3 consecutive scanned pages; click "Export Multi-Page Document".
...
test("CF-M04: 3 accepted pages export as one multi-page PDF via the native share sheet", async ({ appPage: page, device }) => {
  ...
  await expect(page.getByText("Document Pages (3)")).toBeVisible();
  await expect(page.locator('img[alt="Pg"]')).toHaveCount(3);

  await page.getByText("Export Multi-Page Document to Phone", { exact: false }).click();
